...
from config.paths import DATA_PATH, RECORD_FILE
...
def get_new_or_updated_pdfs() -> list[str]:
    """
    Compare current files with previously indexed files.

    Returns a list of new or modified PDF filenames.
    """
    if os.path.exists(RECORD_FILE):
        with open(RECORD_FILE, "r") as f:
            previously_indexed = json.load(f)
    else:
        previously_indexed = {}
    ...
    for filename in os.listdir(DATA_PATH):
        ...
        if (
            filename not in previously_indexed
            or previously_indexed[filename] != file_hash
        ):
